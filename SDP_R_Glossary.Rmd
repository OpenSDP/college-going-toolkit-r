...
R has some example data built in which we will use in the examples below. You can 
load it like this. 
...
#.keep_all tells R we want to return all columns
```
...
Here is how we might do an operation without a pipeline:
...
We have to read this function inside out which means looking into the call and 
trying to find where it starts (hint: `filter`), then reading the arguments 
outward from there. For complex arguments this is hard. Alternatively we might 
see this written more naturally and more readably this way:
...
The above is easier to read, but it creates a lot of clutter in our workspace. 
After this series of commands we have added `subData` and `cylSum` to the memory 
in R. Later in our script will we remember what these objects are for? Will we 
remember to clean them up at the end of the command? An advantage of writing 
functions is that they do not show temporary data objects to the user and they 
clean up after themselves. 

Pipelines allow us to do gain the tidiness of functions with the readability and 
...
word `THEN`. This becomes a very readable block of text that is easy to follow, 
easy to read, and easy to anticipate the expected output from. 
...
Instead of having to type `mtcars$mpg <- sqrt(mtcars$mpg)` we simply had to type 
the object we wanted to transform, the pipe operator, and the function we wanted 
to apply. 
...
and the modular way it allows users to assemble complex graphics from simple  
parts. 
