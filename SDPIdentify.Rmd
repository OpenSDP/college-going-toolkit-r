...
\begin{tabular}{p{.15\paperwidth} p{.4\paperwidth} p{.03\paperwidth} 
p{.03\paperwidth}  p{.03\paperwidth}}
...
\cellcolor{blue!25} & \cellcolor{red!25} & 5 \\ 
Student\_School\_Year & School year and attendance data for students. & 
\cellcolor{blue!25} & \cellcolor{red!25} & 6 \\ 
Student\_School\_Enrollment & School enrollment/withdrawal data for students. &
\cellcolor{blue!25} & \cellcolor{red!25} & 8 \\ 
Student\_Class\_Enrollment & Class enrollment, grades, and credits earned for 
students & \cellcolor{blue!25} & \cellcolor{red!25} & 9 \\ 
Student\_Test\_Scores & Standardized test data for students (state standardized 
tests, advanced placement, SAT, ACT, etc.). Every attempt at a test by 
a student should  be recorded. & \cellcolor{blue!25} & \cellcolor{red!25} & 10 \\ 
...
outcomes. & \cellcolor{blue!25} &  & 12 \\ \hline

\textbf{SCHOOL DATA FILES} & & & & \\

School & Location and classification of schools. & \cellcolor{blue!25} &
\cellcolor{red!25} & 14 \\ 
Class & Class level scheduling data. &  & \cellcolor{red!25} & 16 \\ \hline

\textbf{STAFF DATA FILES} & & & & \\

Staff\_Attributes & Demographic and recruitment data of staff. & & 
\cellcolor{red!25} & 18 \\ 
Staff\_School\_Year & Pay, experience, school placement, and job codes of 
staff. &  & \cellcolor{red!25} & 20 \\ 
Staff\_Degrees & Educational achievement of staff. Each degree a staff member 
received should be recorded once. &  & \cellcolor{red!25} & 21 \\ 
Staff\_Certification & Teaching Certifications received by staff. &  &
\cellcolor{red!25} & 22 \\ \hline
...
The tables that follow will describe the data files you will need.
...
\item[] 0 = not Hispanic
\item[] 1 = Hispanic
\end{itemize} & Student ethnicity. For systems where race and ethnicity are treated 
...
Generally these classifications follow the standard special education classification. & 
...
Some Systems refer to this category as Limited English Proficiency (LEP) or 
...
days\_suspended \_out\_of\_school & number of days & Total number of days over 
the school year a student experienced out of school suspension & 2 Good to Have & 
\\ \hline

days\_suspended \_in\_school & number of days & Total number of days over 
...
system or moving from one middle school to a different middle school). 
...
if they were enrolled at the same school the year before). The latter case is 
preferred. It is also what we observe in most agencies. & 4 Absolutely Necessary
& \\ \hline
...
describing enrollment reason into the school, if available. & 1 Not Essential & 
\\ \hline
...
the student was enrolled at a given school. The system's data sources 
...
the student was present at a given school. The system's data sources 
may report this directly or you may calculate it based on enrollment dates. & 
2 Good to Have & \\ \hline

days\_absent & number of days & Number of school days during the school year 
the student was absent at a given school. The system's data sources 
may report this directly or you may calculate it based on enrollment dates. & 
2 Good to Have & \\ \hline
...
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & The date the student enrolled in the class. 
...
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & The date the student withdrew from class. 
...
received in the class. (``final'' means last, cumulative grade assigned). 
...
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & The exact date (or at a minimum school year) 
...
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & Start of enrollment. & 4 Absolutely Necessary & 
\\ \hline

enrollment\_end &  \begin{itemize} \setlength\itemsep{-2pt} 
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & End of enrollment. & 4 Absolutely Necessary & 
\\ \hline
...
data for the student. & 4 Absolutely Necessary. & \\ \hline
...
\begin{itemize} \setlength\itemsep{-2pt} 
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & Student's high school graduation date; if 
...
\item[] date format
\item[] (yyyy-mm-dd) \end{itemize} & Student's college graduation date. & 
2 Good to Have & \\ \hline
...
\item[] 020 = second semester course
...
Separate variables that describe different components of salary. For example: 
salary\_group\_degree, salary\_group\_experience, salary\_group\_nbct etc. 
Identifies the teacher's base salary, and generally includes a component 
...
important to capture in the codebook. & Only necessary if we do not know teaching 
...
in the secondary school. & 2 Good to Have & \\ \hline
...
degree\_major & use local codes & Description of the degree major or subject. & 
2 Good to Have & \\ \hline
