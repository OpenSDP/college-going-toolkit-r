...
chartData <- 
  bind_rows(plotdf %>% select(first_hs_name, enrl_1oct_grad_yr1_2yr, enrl_1oct_grad_yr1_4yr, 
                               hs_diploma) %>%
              group_by(first_hs_name) %>% 
  ...
  plotdf %>% select(enrl_1oct_grad_yr1_2yr, enrl_1oct_grad_yr1_4yr, 
                               hs_diploma) %>% 
    summarize_all(funs(sum), na.rm=TRUE) %>% 
    mutate(first_hs_name = "Agency AVERAGE")
  )

chartData <- chartData %>% gather(key = outcome, 
                             value = measure, -first_hs_name, -hs_diploma)

```

```{r}

ggplot(chartData, aes(x = first_hs_name, y = measure/hs_diploma, 
